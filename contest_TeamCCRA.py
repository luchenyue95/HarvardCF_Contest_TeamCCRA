...
ax2.set_xlabel("Wilderness_Area")
...
ind3 = np.arange(N2)    
Cover2 = []
labels3 = list(sorted(df.Soil_Type.unique()))

for cover_number2 in sorted(df.Cover_Type.unique()):
    cover2 = []
    for i in range(len(df.Soil_Type.unique())):
        i += 1
        cover.append(df[(df.Soil_Type==str(i)) & (df.Cover_Type == cover_number2)].count()[0])
    Cover2.append(cover2)

f, ax3 = plt.subplots(1, figsize=(8,5))

bar_width = 0.5

tick_pos = [n+(bar_width/2) for n in ind2] 

ax3.bar(ind2, Cover2[0], width=bar_width, label='COVER_TYPE_1', alpha=0.5,color='red')

ax3.bar(ind2, Cover2[1], width=bar_width,bottom=Cover[0], label='COVER_TYPE_2', alpha=0.5, color='orange')

ax3.bar(ind2, Cover2[2], width=bar_width,
        bottom=[j+k for j,k in zip(Cover[0],Cover[1])], 
        label='COVER_TYPE_3', alpha=0.5, color='yellow')

ax3.bar(ind2, Cover2[3],width=bar_width,
        bottom=[j+k+l for j,k,l in zip(Cover[0],Cover[1],Cover[2])], 
        label='COVER_TYPE_4', alpha=0.5, color='green')

ax3.bar(ind2, Cover2[4],width=bar_width,
        bottom=[j+k+l+m for j,k,l,m in zip(Cover[0],Cover[1],Cover[2],Cover[3])], 
        label='COVER_TYPE_5', alpha=0.5, color='blue')

ax3.bar(ind2, Cover2[5], width=bar_width,
        bottom=[j+k+l+m+n for j,k,l,m,n in zip(Cover[0],Cover[1],Cover[2],Cover[3],Cover[4])], 
        label='COVER_TYPE_6', alpha=0.5, color='purple')

ax3.bar(ind2, Cover2[6],width=bar_width,
        bottom=[j+k+l+m+n+o for j,k,l,m,n,o in zip(Cover[0],Cover[1],Cover[2],Cover[3],Cover[4],Cover[5])], 
        label='COVER_TYPE_7', alpha=0.5, color='black')

plt.xticks(tick_pos, labels2)

ax3.set_xlabel("Soil_Type")
ax3.set_ylabel("Count")
ax3.set_title("Differential Cover Type Distribution in Wilderness Areas",fontsize = 18,y=1.01)
plt.legend(loc='upper right',frameon = True)
plt.xlim([min(tick_pos)-bar_width, max(tick_pos)+bar_width])
fig3 = ax3.get_figure()
fig3.savefig("/Users/chenyuelu/Desktop/computefest/Python/contest/stackedBars3")
